...
fn contrast() {
    ...
    for p in PALETTES {
        for accent in std::iter::once(None).chain(p.accents.iter().map(|a| Some(a.id))) {
            ...
            for (prefix, group) in [("surface", c.surface), ("surface_alt", c.surface_alt)] {
                for (name, bg) in [
                    ("background", group.background),
                    ("surface", group.surface),
                    ("raised", group.raised),
            ...
            for (prefix, action, fg) in [
                ("primary", c.primary, c.text.on_primary),
                ("secondary", c.secondary, c.text.on_secondary),
            ] {
                for (name, bg) in [
            ...
            for (status, fg) in [
                ...
                pairs.push((format!("{status}/background"), fg, c.surface.background));
            }
            for (name, fg, bg) in pairs {
                let r = ratio(fg, bg, c.surface.background);
                println!(
                    "{}\t{}\t{name}\t{r:.3}\t{}",
                    t.id(),
                    accent.unwrap_or(""),
